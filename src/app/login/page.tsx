...
export default function LoginPage() {
  ...
  const isRestrictedRole = activeRole === "coordinator" || activeRole === "ngodashboard";
  ...
  const handleEmailLogin = async () => {
    ...
    // Hardcoded credential check for Admin/SuperAdmin
    if (activeRole === "admin") {
    ...
    try {
      ...
      // Verify role in database for restricted roles
      const userDoc = await getDoc(doc(db, "users", cred.user.uid));
      const userData = userDoc.data();

      if (!userData) {
        if (isAdminRole) {
          // Create admin record if it doesn't exist yet but credentials were correct
        ...
        // Allow higher privileges (admin/superadmin) to test specific lower roles if they want,
        // or just strictly enforce if requested role is admin/coordinator.
        if (activeRole === "user") {
          // If they just logged into the 'User' tab, that's fine. Drop them at the User route.
          // Or let them pass if they are actually an admin.
        } else {
      ...
      // Direct redirection (2FA removed)
    ...
    try {
      ...
      if (!userDoc.exists()) {
        await setDoc(doc(db, "users", cred.user.uid), {
          ...
          role: "user", // Google login always defaults to user role
          isBanned: false,
          createdAt: new Date().toISOString(),
        });
      } else if (userDoc.data().role !== "user") {
        // If they have a different role, they must use email login
